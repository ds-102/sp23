...
class BanditEnv:
    """Environment for multi-armed bandits"""
    ...
    def initialize(self, make_plot=1, alg_defined=0, alg=None):
        ...
        if make_plot:
            ...
            self.figure.suptitle("Bandits Demo")
            self._plot_dists()
    ...
    def _make_buttons(self, true_ax):
        ...
        for arm in range(self.num_arms):
            self.buttons.append(
                Button(
                    plt.axes(locations[arm]),
                    str(arm),
                    color=self.COLOURS[arm],
                    hovercolor=self.COLOURS2[1],
                )
            )

            self.buttons[-1].on_clicked(
                functools.partial(self._interactive_pull_arm, pull=arm)
            )

        self.buttons.append(
            CheckButtons(
                plt.axes([0.2, 0.88, 0.12, 0.12], facecolor="w"),
                ("Distributions",),
                [self.show_dists],
            )
        )
        ...
        self.buttons.append(
            CheckButtons(
                plt.axes([0.5, 0.88, 0.12, 0.12], facecolor="w"),
                ("Regret",),
                [self.show_regret],
            )
        )
        ...
        if not self.alg_defined:
            self.buttons.append(
                Button(
                    plt.axes([0.775, 0.915, 0.12, height], facecolor="w"),
                    "Reset",
                    color=self.COLOURS2[0],
                    hovercolor=self.COLOURS2[1],
                )
            )
    ...
    def _get_Button_Locations(self, true_ax):
        ...
        width, h = self.figure.transFigure.inverted().transform(
            true_ax.transData.transform([axes_width, 0])
        ) - self.figure.transFigure.inverted().transform(
            true_ax.transData.transform([0, 0])
        )

        w, height = self.figure.transFigure.inverted().transform(
            true_ax.transData.transform([0, axes_height])
        ) - self.figure.transFigure.inverted().transform(
            true_ax.transData.transform([0, 0])
        )
        ...
        for arm in range(self.num_arms):
            ...
            t1, t2 = self.figure.transFigure.inverted().transform(
                true_ax.transData.transform([location - axes_width / 2.0, -8.5])
            )
            locations.append([t1, t2, width, height])

        if self.show_dists:
            t1, t2 = self.figure.transFigure.inverted().transform(
                true_ax.transData.transform([0.1, -8.5])
            )
            locations.append([t1, t2, 2 * width, height])
        elif not self.show_dists:
            t1, t2 = self.figure.transFigure.inverted().transform(
                true_ax.transData.transform([-0.5 - 2 * axes_width, -12.5])
            )
            locations.append([t1, t2, 2 * width, height])
            ...
            if not self.alg_defined:
                if len(self.rewards[arm]) > 0:
                    self.plot_content.extend(
                        true_ax.plot(
                            [location for r in self.rewards[arm]],
                            self.rewards[arm],
                            "o",
                            ms=5,
                            mfc="None",
                            mec=self.COLOURS[arm],
                            mew=0.5,
                        )
                    )
...
class Interactive_UCB_Algorithm:
    ...
    def _make_plots(self, true_ax):
        ...
        for arm in range(self.bandit_env.num_arms):
            ...
            if len(self.bandit_env.rewards[arm]) > 0:
                self.bandit_env.plot_content.extend(
                    true_ax.plot(
                        [location, location],
                        [
                            np.mean(self.bandit_env.rewards[arm]),
                            self.upper_confidence_bounds[arm],
                        ],
                        lw=2.5,
                        c=self.bandit_env.COLOURS[arm],
                    )
                )
                ...
                self.bandit_env.plot_content.extend(
                    true_ax.plot(
                        [location], [50], ".", ms=15, c=self.bandit_env.COLOURS[arm]
                    )
                )
        return
    ...
    def _make_buttons(self, true_ax):
        ...
        self.bandit_env.buttons.append(
            Button(
                plt.axes(locations[-1]),
                self.name,
    ...
    def recalculate_bounds(self):
        t = self.bandit_env.t
        arm, self.upper_confidence_bounds = self.alg(
            t,
            self.bandit_env.standard_deviations,
            self.bandit_env.times_pulled,
            self.bandit_env.rewards,
        )
        return
    ...
    def _pull_arm_alg(self, event):
        ...
        assert (
            arm in range(self.num_arms)
            and len(self.upper_confidence_bounds) == self.num_arms
        )
...
class Interactive_TS_Algorithm:
    ...
    def recalculate_bounds(self, start=0):
        ...
        arm, self.samples, self.means, self.vars = self.alg(
            t,
            var,
            self.bandit_env.times_pulled,
            self.bandit_env.rewards,
